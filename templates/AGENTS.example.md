...
- Observatório público organizado por **temas do estado atual do conhecimento**, com `Atualizado em DD/MM/AAAA`; versões técnicas como `Snapshot 0.1` ficam internas por padrão; cada tema explicita síntese, mudanças analisadas, questões em aberto, fontes e conteúdos relacionados, com blocos específicos do domínio.
...
## Arquitetura de ambientes v2

Este Projeto mantém identidade `PRJ/EA/F` independente de dispositivo, executor, memória ou fonte.

- Superfície: mobile/web/desktop;
- Executor: Chat/Work/Codex;
- Vínculo: declarar primeiro `B0` (sem Projeto ChatGPT) ou `B1` (com Projeto ChatGPT); somente em `B1` declarar `C2` ou `C3`;
...
Se houver vínculo `B1` e o Projeto ChatGPT estiver em `C3`, Work não estará disponível dentro dele; em `B0`, C2/C3 é não aplicável. Handoff explícito é obrigatório quando o executor de destino não herdar o contexto necessário.

Filesystem local é working copy; nunca a única fonte necessária à continuidade.

`EXECUTION_ENVIRONMENT_PROFILE.json` schema v2 registra a política local.
